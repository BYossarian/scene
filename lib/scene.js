...
function render(surfaces, lights, renderTarget, options) {
    ...
    const screenWidth = options.screenWidth || DEFAULT_RESOLUTION.WIDTH;
    const screenHeight = options.screenHeight || DEFAULT_RESOLUTION.HEIGHT;

    if (renderTarget.length !== (screenWidth * screenHeight * 4)) {
        throw new Error(`renderTarget has size ${renderTarget.length} which doesn't match requested resolution of ${screenWidth} x ${screenHeight}`);
    }
    ...
    const xOffset = 0.5 - screenWidth / 2;
    const yOffset = screenHeight / 2 - 0.5;
    const z = screenHeight / Math.tan((options.verticalFOY || Math.PI / 4) / 2);
    const worldFront = _scale(z, cameraFront);

    const scene = {
            lights: lights,
            surfaces: surfaces,
            ambientLight: options.ambientLight || 0.1,
    ...
    for (let j = 0; j < screenHeight; j++) {
        for (let i = 0; i < screenWidth; i++) {
            ...
            const color = _traceRay(scene, viewRay, 0, Infinity, DEFAULT_RECURSION_DEPTH) || scene.background;
